...
class ModbusThread(threading.Thread):
    def __init__(self):
        ...
        self.tempo_botao_pressionado = {}  # Armazena o tempo de pressão de cada botão
    ...
    def atualizar(self, botao: str, valor: str, label: list, view: bool):
        if valor == self.valor_quando_apertado and botao not in self.lista_apertado:
            ...
            self.tempo_botao_pressionado[botao] = time.time()  # Armazena o tempo de pressão
            ...
            if botao in self.tempo_botao_pressionado:
                tempo_pressionado = time.time() - self.tempo_botao_pressionado[botao]
                if tempo_pressionado > 2:
                    # Adiciona DUAS entradas negativas ao banco de dados para decrementar o contador em 2 unidades
                    cur.execute(inserir_dados, (self.dia_hoje, self.mes_hoje, self.ano_hoje,
                                                self.time, f"-{botao}"))  # insere botao com valor negativo (1ª unidade)
                    cur.execute(inserir_dados, (self.dia_hoje, self.mes_hoje, self.ano_hoje,
                                                self.time, f"-{botao}"))  # insere botao com valor negativo (2ª unidade)
                    conexao.commit()

                    # Atualiza o contador na interface gráfica (subtrai 2 unidades)
                    self.total_var[botao] -= 2
                    label[2].config(text=f"Total: {self.total_var[botao]}", bg='gray30')

            del self.tempo_botao_pressionado[botao]  # Remove o tempo de pressão
    ...
    def cancelar_ultima_acao(self):
        cur.execute("SELECT rowid, doenca FROM dados ORDER BY rowid DESC LIMIT 1")
        ultima_entrada = cur.fetchone()

        if ultima_entrada:
            rowid, botao = ultima_entrada
            cur.execute("DELETE FROM dados WHERE rowid = ?", (rowid,))
            conexao.commit()

            cur.execute(pegaQntPato, (botao, datetime.now().strftime("%d")))
            novo_total = cur.fetchone()[0]

            if botao in self.dicio_botao_label:
                label_info = self.dicio_botao_label[botao]
                label_info[2].config(text=f"Total: {novo_total}", bg='gray30')
                label_info[0].itemconfig(label_info[1], fill='gray30')

            self.mostrar_mensagem_temporaria("Última ação desfeita!", 'orange')
        else:
            self.mostrar_mensagem_temporaria("Nenhuma ação para desfazer!", 'red')

    def mostrar_mensagem_temporaria(self, mensagem, cor):
        erro_txt.config(text=mensagem, fg=cor)  # Exibe a mensagem
        tela.update()  # Força a atualização da tela

        # Agenda a remoção da mensagem após 1.2 segundos (1200 ms)
        tela.after(1200, lambda: erro_txt.config(text=""))
...
def baixa_pdf():
    ...
    modbus_thread.mostrar_mensagem_temporaria(f"{str(resultado4)}", 'red')
...
# Função para excluir o banco de dados inteiro
def excluir_banco_dados():

    modbus_thread.total_var = {"0": 0, "1": 0, "2": 0, "3": 0}
    cur.execute("DELETE FROM dados")  # Exclui todos os registros da tabela
    conexao.commit()

    # Reseta os contadores na interface gráfica
    for botao, label_info in modbus_thread.dicio_botao_label.items():
        label_info[2].config(text="Total: 0", bg='gray30')
        label_info[0].itemconfig(label_info[1], fill='gray30')

    modbus_thread.mostrar_mensagem_temporaria("Banco de dados excluído!", 'red')
...
erro_txt.grid(column=1, row=5, padx=10, pady=0, columnspan=5, sticky="W")
...
excluir_banco = Button(tela, text="Excluir Banco", bg='red', fg='white', command=excluir_banco_dados)
excluir_banco.grid(column=0, row=5, padx=10, pady=0, sticky="W")
